...
/// Helper struct for JSON serialization/deserialization.
/// Handles computed `success` field and Microseconds conversion.
#[derive(Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct BuildResultJson {
    #[serde(flatten)]
    inner: BuildResultInner,
    start_time: DaemonTime,
    stop_time: DaemonTime,
    #[serde(skip_deserializing, default)]
    success: bool,
    times_built: DaemonInt,
...
impl Serialize for BuildResult {
        ...
        S: Serializer,
    {
        BuildResultJson {
            ...
            success: matches!(&self.inner, BuildResultInner::Success(_)),
            times_built: self.times_built,
            cpu_user: self.cpu_user.map(|m| m.0),
            cpu_system: self.cpu_system.map(|m| m.0),
        }
        .serialize(serializer)
    }
}
...
impl<'de> Deserialize<'de> for BuildResult {
    ...
    where
        D: Deserializer<'de>,
    {
        let raw = BuildResultJson::deserialize(deserializer)?;
        Ok(BuildResult {
            inner: raw.inner,
            times_built: raw.times_built,
            start_time: raw.start_time,
            stop_time: raw.stop_time,
            cpu_user: raw.cpu_user.map(Microseconds),
            cpu_system: raw.cpu_system.map(Microseconds),
        })
    }
}
