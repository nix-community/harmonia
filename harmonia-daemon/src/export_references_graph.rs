    ...
    for output in built_outputs {
        ...
        if !disallowed_requisites.is_empty() || allowed_requisites.is_some() {
            ...
            for ref_sp in &output.references {
                ...
                tokio::task::spawn_blocking(move || {
                    let db = db.blocking_lock();
                    compute_fs_closure(&db, &sd, &ref_sp, &mut local_closure).map(|_| local_closure)
                })
                .await
...
mod tests {
    ...
    use super::*;
    use harmonia_store_derivation::derivation::{DerivationOutput, DerivationT, StructuredAttrs};
