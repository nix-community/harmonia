//! Filesystem-backed [`FileSystemSource`] and [`FileSystemSink`] over the
//! capability-confined async [`cap`] surface.
//!
...
#[cfg(unix)]
fn is_executable(meta: &cap::Metadata) -> bool {
    use cap::MetadataExt;
    meta.mode() & 0o111 != 0
}

#[cfg(not(unix))]
fn is_executable(_meta: &cap::Metadata) -> bool {
    false
}
